    ...
    int* pProcessId)
{
	...
	LOGE("xingzengmou: %s\n", cmd);
        int i = execl(cmd, cmd, arg0, arg1, NULL);
	LOGE("result = %d", i);
        exit(-1);
    } else {
        *pProcessId = (int) pid;
...
#if 0
static jstring android_os_Exec_exeCommand(JNIEnv *env, jobject clazz, jstring cmd) {
	const char *pcmd = env->GetStringUTFChars(cmd, NULL);
        LOGE("xingzengmou: %s", pcmd);
        if (!pcmd) {
        ...
        if (pipe(fd) < 0) {
                LOGE("pipe error");
                return env->NewStringUTF("Error: pipe error");
        }
        pid = fork();
        if (pid < 0) {
                LOGE("fork error");
                return env->NewStringUTF("Error: fork error");
        }
        if (pid > 0) {
                close(fd[1]);
                while ((n = read(fd[0], buf + count, len)) > 0 && count > len) count += n;
                close(fd[0]);
		LOGE("xingzengmou: %s", buf);
        ...
        if (pid == 0) {
                ...
                if (fd[1] != STDOUT_FILENO) {
                        if (dup2(fd[1], STDOUT_FILENO) != STDOUT_FILENO) {
                                return env->NewStringUTF("Error: dup2 error");
                        }
                        close(fd[1]);
                }
		if (system("su") == -1) return env->NewStringUTF("Error: su error");
		if (system(pcmd) == -1) return env->NewStringUTF("Error: execule command error");
               // if (execl("/system/bin/sh", "sh", "-c", pcmd, (char*)0) == -1) return env->NewStringUTF("Error: execl error");
        }
...
{
    jclass _clazz = env->FindClass("java/io/FileDescriptor");
    ...
    class_fileDescriptor = (jclass) env->NewGlobalRef(_clazz);
...
static const char *classPathName = "com/only/jni/Console";

static JNINativeMethod method_table[] = {
    { "createSubprocess", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;[I)Ljava/io/FileDescriptor;",
        (void*) android_os_Exec_createSubProcess },
    /*{ "exeCommand", "(Ljava/lang/String;)Ljava/lang/String;",
	(void*) android_os_Exec_exeCommand },*/
/*
